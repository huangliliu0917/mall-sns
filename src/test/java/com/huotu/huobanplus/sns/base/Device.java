...
public class Device {
     ...
     * @return
     */
    public static ResultMatcher SnsStatus(AppCode appCode) {
        return new ResultMatcher() {
            @Override
            public void match(MvcResult result) throws Exception {
                status().isOk().match(result);
                jsonPath("$.systemResultCode").value(AppCode.SUCCESS.getValue()).match(result);
...
//        builder.param("timestamp","123");
        ...
        builder.param("customerId", String.valueOf(customerId));
